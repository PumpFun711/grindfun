const World = {
  ...
  // Merged geometry chunks for performance
  chunkMeshes: [],
  ...
  makeTexture(blockType) {
    ...
    const size = 32; // Smaller texture = much better performance
    ...
    // Simple noise pattern per block
    switch(blockType) {
      case 0: // Dirt
        for(let i=0;i<10;i++){
          ctx.fillStyle = i%2===0?darkC:lightC;
          ctx.fillRect(Math.random()*size|0,Math.random()*size|0,(Math.random()*4+1)|0,(Math.random()*2+1)|0);
        }
        break;
      case 1: // Grass top
        ctx.fillStyle='#4a8c2e'; ctx.fillRect(0,0,size,size);
        for(let i=0;i<8;i++){
          ctx.fillStyle=i%2?'#3a7020':'#6ab840';
          ctx.fillRect(Math.random()*size|0,Math.random()*size|0,(Math.random()*4+1)|0,(Math.random()*3+1)|0);
        }
        break;
      case 2: // Sand
        for(let i=0;i<10;i++){
          ctx.fillStyle=i%2?'#c8b060':'#e8d48a';
          ctx.fillRect(Math.random()*size|0,Math.random()*size|0,(Math.random()*3+1)|0,1);
        }
        break;
      case 3: // Wood top (rings)
      ...
      case 5: // Coal
        ...
        [[4,4,7,5],[14,9,5,6],[5,16,7,5],[17,17,6,4]].forEach(([x,y,w,h])=>{
          ctx.fillStyle='#111'; ctx.fillRect(x,y,w,h);
          ctx.fillStyle='#444'; ctx.fillRect(x,y,w,1);
        });
        break;
      case 6: // Iron
        ctx.fillStyle='#6a6a6a'; ctx.fillRect(0,0,size,size);
        [[3,3,5,4],[13,6,5,4],[4,14,5,4],[15,15,5,4],[9,9,4,3]].forEach(([x,y,w,h])=>{
          ctx.fillStyle='#c0896e'; ctx.fillRect(x,y,w,h);
          ctx.fillStyle='#e0a98e'; ctx.fillRect(x,y,w,1);
        });
        break;
      case 7: // Gold
        ctx.fillStyle='#6a6a6a'; ctx.fillRect(0,0,size,size);
        [[3,3,5,4],[13,6,5,4],[4,14,5,4],[15,15,5,4],[9,9,4,3]].forEach(([x,y,w,h])=>{
          ...
          ctx.fillStyle='#f0d060'; ctx.fillRect(x,y,w,1);
        });
        break;
      case 8: // Redstone
        ...
        [[3,3,5,4],[13,6,5,4],[4,14,5,4],[15,15,5,4],[9,9,4,3]].forEach(([x,y,w,h])=>{
          ...
          ctx.fillStyle='#ff4422'; ctx.fillRect(x,y,w,1);
        });
        break;
      case 9: // Lapis
        ctx.fillStyle='#5a5a6a'; ctx.fillRect(0,0,size,size);
        [[3,3,6,5],[13,6,6,5],[4,14,6,5],[15,15,6,5],[8,8,5,4]].forEach(([x,y,w,h])=>{
          ctx.fillStyle='#1a3db5'; ctx.fillRect(x,y,w,h);
          ctx.fillStyle='#4466ff'; ctx.fillRect(x,y,w,1);
        });
        break;
      case 10: // Emerald
        ctx.fillStyle='#444'; ctx.fillRect(0,0,size,size);
        [[3,2,6,6],[14,5,6,6],[3,14,6,6],[14,14,6,6],[9,9,5,5]].forEach(([x,y,w,h])=>{
          ctx.fillStyle='#00b865'; ctx.fillRect(x,y,w,h);
          ctx.fillStyle='#00ff88'; ctx.fillRect(x,y,w,1); ctx.fillRect(x,y,1,h);
        });
        break;
      case 11: // Diamond
        ctx.fillStyle='#333344'; ctx.fillRect(0,0,size,size);
        [[3,2,6,5],[14,5,6,5],[3,14,6,5],[14,14,6,5],[9,9,6,5]].forEach(([x,y,w,h])=>{
          ctx.fillStyle='#2aafd4'; ctx.fillRect(x,y,w,h);
          ctx.fillStyle='#88eeff'; ctx.fillRect(x,y,w,1); ctx.fillRect(x,y,1,h);
        });
        break;
      case 12: // GrindFun
        ctx.fillStyle='#111122'; ctx.fillRect(0,0,size,size);
        [[2,2,8,6],[14,3,8,6],[2,14,8,6],[14,14,8,6],[7,7,8,6]].forEach(([x,y,w,h])=>{
          ctx.fillStyle='#00aa55'; ctx.fillRect(x,y,w,h);
          ctx.fillStyle='#00ff88'; ctx.fillRect(x,y,w,2); ctx.fillRect(x,y,2,h);
          ctx.fillStyle='rgba(0,255,136,0.5)'; ctx.fillRect(x+2,y+2,3,3);
        });
        break;
      case 13: // Leaves
        ctx.fillStyle='#2d7a2d'; ctx.fillRect(0,0,size,size);
        for(let i=0;i<10;i++){
          ctx.fillStyle=i%3?'#1a5a1a':i%3===1?'#3a9a3a':'#4ab040';
          ctx.fillRect(Math.random()*size|0,Math.random()*size|0,(Math.random()*5+2)|0,(Math.random()*4+1)|0);
        }
        break;
    }
  ...
  // Shared material cache — reuse materials across blocks
  matCache: new Map(),
  ...
  addBlockMesh(x, y, z, blockType, geo) {
    ...
    const mats = [
      this.getMat(blockType,'side'),
      this.getMat(blockType,'side'),
      this.getMat(blockType,'top'),
      this.getMat(blockType,'bottom'),
      this.getMat(blockType,'side'),
      this.getMat(blockType,'side'),
    ];
    ...
    mesh.matrixAutoUpdate = false; // Big perf win — static blocks don't need to recalc matrix every frame
    mesh.updateMatrix();
    this.scene.add(mesh);
  ...
  showCrack(x, y, z, progress) {
    ...
    const pts=[[[10,5,18,14]],[[10,5,18,14],[4,15,14,26]],[[10,5,18,14],[4,15,14,26],[20,6,26,21]],[[10,5,18,14],[4,15,14,26],[20,6,26,21],[2,2,29,29]],[[10,5,18,14],[4,15,14,26],[20,6,26,21],[2,2,29,29],[16,2,6,30]]];
