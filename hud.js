const HUD = {
  ...
  buildShop(currentTier, currentPoints) {
    // Update sidebar shop
    const sidebar = document.getElementById('shop-list');
    if (sidebar) {
      sidebar.innerHTML = this.shopHTML(currentTier, currentPoints);
    }
    // Update overlay shop
    const overlay = document.getElementById('shop-overlay-items');
    if (overlay) {
      overlay.innerHTML = this.shopOverlayHTML(currentTier, currentPoints);
    }
  },

  shopHTML(currentTier, currentPoints) {
    return PICKS.map((p, i) => {
      const owned    = i <= currentTier;
      const isNext   = i === currentTier + 1;
      const canAfford = currentPoints >= p.cost && isNext;
      const label    = owned ? `✓ ${p.name}` : `${p.name} — ${p.cost.toLocaleString()} pts`;
      return `<button
  ...
  shopOverlayHTML(currentTier, currentPoints) {
    const EMOJIS = ['🪵','🪨','⚙️','✨','💎','⛏'];
    return PICKS.map((p, i) => {
      const owned     = i <= currentTier;
      const isNext    = i === currentTier + 1;
      const canAfford = currentPoints >= p.cost && isNext;
      return `<div class="shop-card${owned ? ' owned' : ''}${canAfford ? ' affordable' : ''}">
        <div class="shop-card-emoji">${EMOJIS[i]}</div>
        <div class="shop-card-name" style="color:${p.color}">${p.name} Pickaxe</div>
        <div class="shop-card-power">⚡ ${p.power}× power</div>
        <div class="shop-card-cost">${p.cost === 0 ? 'FREE' : p.cost.toLocaleString() + ' pts'}</div>
        ${owned
          ? `<div class="shop-card-owned">✓ OWNED</div>`
          : `<button class="shop-card-btn" ${!canAfford ? 'disabled' : ''} onclick="Network.buyPickaxe(${i});Game.closeShop()">
              ${canAfford ? 'BUY NOW' : currentPoints < p.cost ? 'NOT ENOUGH PTS' : 'LOCKED'}
            </button>`
        }
      </div>`;
    }).join('');
  ...
  showFloatText(text, color) {
    ...
    if (!document.getElementById('float-style')) {
      ...
      style.textContent = `@keyframes floatCenter{0%{opacity:1;transform:translate(-50%,-50%) scale(1.3)}30%{opacity:1;transform:translate(-50%,-60%) scale(1)}100%{opacity:0;transform:translate(-50%,-80%) scale(0.8)}}`;
